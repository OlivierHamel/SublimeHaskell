...
def call_no_wait(command, split_lines = False, **popen_kwargs):
    return call_no_wait_with_input(command, '', split_lines, **popen_kwargs)
...
def call_and_wait_with_input(command, input_string, split_lines = False, **popen_kwargs):
    ...
    retVal = call_no_wait_with_input(command, '', split_lines, **popen_kwargs)
    if split_lines:
        return retVal
    
    stdout, stderr, process = retVal
    exit_code = process.wait()
    return (exit_code, crlf2lf(decode_bytes(stdout)), crlf2lf(decode_bytes(stderr)))

def call_no_wait_with_input(command, input_string, split_lines = False, **popen_kwargs):
